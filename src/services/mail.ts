...
// Shared HTML wrapper
function htmlWrap(bodyHtml: string) {
  return `
    <!DOCTYPE html>
    <html>
    <head>
...
async function safeSend(mailOptions: MailOptions) {
  if (!isMailConfigured()) {
...
// 1. OTP Verification
async function sendGuestVerificationOtp({ to, fullName, otpCode }: OtpMailInput) {
  await safeSend({
    to,
...
async function sendGuestBookingAccessOtp({ to, fullName, otpCode }: OtpMailInput) {
  await safeSend({
    to,
...
async function sendGuestPasswordResetOtp({ to, fullName, otpCode }: OtpMailInput) {
  await safeSend({
    to,
...
async function sendBookingConfirmation({ to, fullName, reservation }: ReservationMailInput) {
  const {
  ...
  const fmtMoney = (n: number | string) => Number(n || 0).toLocaleString('en-US');
...
async function sendCancellationNotice({ to, fullName, reservation, cancelledBy = 'guest', reason }: ReservationMailInput) {
