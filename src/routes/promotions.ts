...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const request = pool.request()
      .input('hotelId', sql.BigInt, hotel_id ? parseInt(String(hotel_id), 10) : null)
      .input('guestId', sql.BigInt, effectiveGuestId ? parseInt(String(effectiveGuestId), 10) : null)
...
router.post('/', requireAdminUser, async (req: Request, res: Response) => {
  try {
    const {
...
router.put('/:id', requireAdminUser, async (req: Request, res: Response) => {
  try {
    const promoId = parseInt(String(req.params.id), 10);
...
router.delete('/:id', requireAdminUser, async (req: Request, res: Response) => {
  try {
    const promoId = parseInt(String(req.params.id), 10);
    if (isNaN(promoId)) return res.status(400).json({ success: false, message: 'Invalid ID' });
...
router.post('/validate-voucher', async (req: Request, res: Response) => {
  try {
    ...
    const pool = getSqlPool();
    const result = await pool.request()
      .input('guestId', sql.BigInt, parseInt(String(effectiveGuestId), 10))
      .input('hotelId', sql.BigInt, parseInt(String(hotel_id), 10))
