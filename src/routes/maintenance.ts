...
const { requireSystemRole } = require('../middleware/auth');
...
// 
router.get('/', requireSystemRole(['FRONT_DESK', 'HK_MANAGER', 'MAINTENANCE', 'MANAGER', 'ADMIN']), async (req: Request, res: Response) => {
  try {
    ...
    if (room_id) {
      ...
      request.input('roomId', sql.BigInt, parseInt(String(room_id), 10));
    }
    if (status) {
    ...
    if (severity) {
      ...
      request.input('severity', sql.VarChar(10), String(severity));
    }
...
// 
router.post('/', requireSystemRole(['FRONT_DESK', 'HK_MANAGER', 'MAINTENANCE', 'MANAGER', 'ADMIN']), async (req: Request, res: Response) => {
  try {
...
// 
router.put('/:id', requireSystemRole(['MAINTENANCE', 'MANAGER', 'ADMIN']), async (req: Request, res: Response) => {
  try {
