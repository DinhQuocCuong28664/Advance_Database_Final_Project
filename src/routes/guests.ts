...
function ensureGuestAccess(req: Request, res: Response, guestId: number) {
...
async function markExpiredRedemptions(pool: any, guestId: number) {
  await pool.request()
...
async function generateRedemptionCode(requestFactory: () => any) {
...
router.get('/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    const pool = getSqlPool();
    const guestId = parseInt(String(req.params.id), 10);
    if (isNaN(guestId)) {
...
router.get('/:id/loyalty-rewards', requireAuth, async (req: Request, res: Response) => {
  try {
    const pool = getSqlPool();
    const guestId = parseInt(String(req.params.id), 10);
    if (isNaN(guestId)) {
...
router.get('/:id/loyalty-redemptions', requireAuth, async (req: Request, res: Response) => {
  try {
    const pool = getSqlPool();
    const guestId = parseInt(String(req.params.id), 10);
    if (isNaN(guestId)) {
