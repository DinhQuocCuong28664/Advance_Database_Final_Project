...
function ensureReservationAccess(req: Request, res: Response, guestId: number | string) {
  ...
  try {
    ...
    if (reservation_id) {
      const accessCheck = await pool.request()
        .input('resvId', sql.BigInt, parseInt(String(reservation_id), 10))
    ...
    let statusClause = '';
    if (status) {
      request.input('status', sql.VarChar(15), String(status).toUpperCase());
