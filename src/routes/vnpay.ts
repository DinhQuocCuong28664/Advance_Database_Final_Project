...
const { requireAdminOrManagerUser } = require('../middleware/auth');
...
router.post('/create-payment', async (req: Request, res: Response) => {
  try {
    ...
    const resvRow = await pool.request()
      ...
      .query(`
        SELECT reservation_code, reservation_status, deposit_amount
        FROM Reservation
    ...
    const reservation = resvRow.recordset[0];
    if (['CANCELLED', 'CHECKED_OUT', 'NO_SHOW'].includes(reservation.reservation_status)) {
      return res.status(400).json({
        success: false,
        message: `Cannot create VNPay payment for reservation with status: ${reservation.reservation_status}`,
      });
    }

    const expectedAmount = Math.round(Number(reservation.deposit_amount || 0));
    const requestedAmount = Math.round(Number(amount));
    if (!(expectedAmount > 0) || requestedAmount !== expectedAmount) {
      return res.status(400).json({
        success: false,
        message: `VNPay amount must match reservation deposit amount. Required: ${expectedAmount}, attempted: ${requestedAmount}`,
      });
    }

    const txnRef = reservation.reservation_code;
...
async function _cancelAbandonedReservation(txnRef: string, reason: string) {
  try {
    const pool = getSqlPool();
...
router.post('/cleanup-abandoned', requireAdminOrManagerUser, async (req: Request, res: Response) => {
  try {
    const pool = getSqlPool();
    ...
    const cancelled = (result.recordset || []).map((r: any) => r.reservation_code);
    res.json({
      success: true,
...
async function _recordVnpayPayment(result: VnpayResult, source: string) {
  try {
    const pool = getSqlPool();
