...
// 
router.get('/staff', requireSystemRole(['FRONT_DESK', 'HK_MANAGER', 'MANAGER', 'ADMIN']), async (_req: Request, res: Response) => {
  try {
...
router.get('/', requireSystemRole(['HOUSEKEEPING', 'HK_MANAGER', 'FRONT_DESK', 'MANAGER', 'ADMIN']), async (req: Request, res: Response) => {
  try {
    ...
    const request = pool.request().input('hotelId', sql.BigInt, parseInt(String(hotel_id), 10));
    let filters = 'WHERE hk.hotel_id = @hotelId';

    if (status) {
      filters += ' AND hk.task_status = @status';
      request.input('status', sql.VarChar(15), String(status));
    }
    if (priority) {
      filters += ' AND hk.priority_level = @priority';
      request.input('priority', sql.VarChar(10), String(priority));
    }
    ...
    // Summary stats
    const stats = await pool.request().input('hotelId', sql.BigInt, parseInt(String(hotel_id), 10)).query(`
    ...
    res.json({
      ...
      summary: stats.recordset.reduce((acc: Record<string, number>, r: any) => { acc[r.task_status] = r.cnt; return acc; }, {}),
      data: result.recordset
    });
  } catch (err: any) {
    res.status(500).json({ success: false, message: err.message });
...
// 
router.post('/', requireSystemRole(['HK_MANAGER', 'MANAGER', 'ADMIN']), async (req: Request, res: Response) => {
  try {
...
// 
router.put('/:id/assign', requireSystemRole(['HK_MANAGER', 'MANAGER', 'ADMIN']), async (req: Request, res: Response) => {
  try {
