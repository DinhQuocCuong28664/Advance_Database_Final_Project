...
function computePromotionDiscount(promotionType: unknown, discountValue: unknown, subtotalAmount: unknown) {
...
async function loadLoyaltyVoucherForBooking(transaction: SqlTransaction, guestId: number | string, hotelId: number | string, loyaltyRedemptionCode: string) {
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const nightCount = nights || Math.round((checkout.getTime() - checkin.getTime()) / (1000 * 60 * 60 * 24));
    ...
    try {
      ...
      for (let i = 0; i < nightCount; i++) {
        ...
        if (lockStatus !== 0) {
          // sp_ReserveRoom already rolled back its savepoint on failure.
          // Roll back the outer Node.js transaction cleanly.
          await transaction.rollback();
