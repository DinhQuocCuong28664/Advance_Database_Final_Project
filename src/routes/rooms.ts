...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const safeLimit = Math.min(parseInt(String(limit || 100), 10) || 100, 500);
...
router.get('/availability', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.request()
      .input('hotelId', sql.BigInt, parseInt(String(hotel_id), 10))
      .input('checkin', sql.VarChar(10), String(checkin))
      .input('checkout', sql.VarChar(10), String(checkout))
      .query(`
    ...
    const availabilityMap = new Map<number | string, any[]>();
    ...
    const rooms = result.recordset.map((room: any) => ({
      ...room,
