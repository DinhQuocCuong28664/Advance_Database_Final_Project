...
router.get('/', requireSystemRole(['FRONT_DESK', 'CASHIER', 'MANAGER', 'ADMIN']), async (req: Request, res: Response) => {
  try {
    const pool = getSqlPool();
    const { reservation_id } = req.query;

    const result = await pool.request()
      .input('resvId', sql.BigInt, reservation_id ? parseInt(String(reservation_id), 10) : null)
      .query(`
...
router.post('/', requireSystemRole(['FRONT_DESK', 'CASHIER', 'MANAGER', 'ADMIN']), async (req: Request, res: Response) => {
  try {
...
// Get invoice detail
router.get('/:id', requireSystemRole(['FRONT_DESK', 'CASHIER', 'MANAGER', 'ADMIN']), async (req: Request, res: Response) => {
  try {
...
router.post('/:id/issue', requireSystemRole(['CASHIER', 'MANAGER', 'ADMIN']), async (req: Request, res: Response) => {
  try {
    const invoiceId = parseInt(String(req.params.id), 10);
    if (Number.isNaN(invoiceId)) {
      return res.status(400).json({ success: false, message: 'Invalid invoice ID' });
