...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    mongoDocs.forEach((doc: any) => { mongoMap[doc.hotel_id] = doc; });

    // Merge
    const hotels = sqlResult.recordset.map((h: any) => ({
      ...h,
      description: mongoMap[h.hotel_id]?.description || null,
      hero_image: mongoMap[h.hotel_id]?.images?.find((i: any) => i.is_hero)?.url || null,
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const hotelId = parseInt(String(req.params.id), 10);
    if (isNaN(hotelId)) {
    ...
    roomFeatures.recordset.forEach((f: any) => {
    ...
    const roomTypeCodes = roomTypes.recordset.map((r: any) => r.room_type_code);
