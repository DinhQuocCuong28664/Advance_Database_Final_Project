Project GlobalLuxuryHotelReservationEngine {
  ...
  MongoDB collections (not modeled as relational tables here):
  - hotel_catalog
  - room_type_catalog
  - amenity_master
  - guest_profile_projection
  - review_summary
  - search_audit_log
  '''
}
...
Table HotelAmenity {
  ...
  amenity_category amenity_category [not null]
...
Table RoomType {
  ...
  has_butler_service boolean [not null, default: false]
  ...
  Note: 'Commercial room type sold to guests'
}
...
Table RoomFeature {
  ...
  Note: 'Feature attached either to a room type or a physical room'
}
...
Table RoomAvailability {
  ...
  Note: 'Critical table for inventory checks and pessimistic locking'
}
...
Table RoomRate {
  ...
  Note: 'Daily sell rate by room type and rate plan'
}
...
Table RateChangeLog {
  ...
  indexes {
    (hotel_id, triggered_at)
  }

  Note: 'Populated by trigger when rate change exceeds policy threshold'
}
...
Table InventoryLockLog {
  ...
  indexes {
    (hotel_id, room_id, stay_date)
  }

  Note: 'Technical/audit table to support concurrency defense'
}
...
Ref: RateChangeLog.hotel_id > Hotel.hotel_id
Ref: RateChangeLog.room_type_id > RoomType.room_type_id
Ref: RateChangeLog.rate_plan_id > RatePlan.rate_plan_id
...
Ref: InventoryLockLog.hotel_id > Hotel.hotel_id
