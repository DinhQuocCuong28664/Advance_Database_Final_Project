...
type SearchDirectionsPreviewProps = {
  hotel: HotelDirections;
...
function createPin(label: string, modifier: string) {
  return L.divIcon({
...
function fitBoundsForPoints(map: L.Map, points: LatLngTuple[]) {
  if (points.length >= 2) {
...
export default function SearchDirectionsPreview({
  ...
  onOpenGoogleMaps,
  onRefreshLocation,
}: SearchDirectionsPreviewProps) {
  const hotelPoint = useMemo(
    (): LatLngTuple => [Number(hotel.latitude), Number(hotel.longitude)],
    [hotel.latitude, hotel.longitude],
  );
  const userPoint = useMemo(
    (): LatLngTuple | null => (userLocation ? [userLocation.latitude, userLocation.longitude] : null),
    [userLocation],
  );
  const mapPoints = useMemo(
    (): LatLngTuple[] => (userPoint ? [hotelPoint, userPoint] : [hotelPoint]),
      ...
      <div className="search-directions-panel__body">
        ...
        <div className="search-directions-map-wrap">
          ...
          >
            ...
            <LeafletMarker position={hotelPoint} icon={hotelIcon}>
              <Popup>{hotel.hotel_name}</Popup>
            </LeafletMarker>
            {userPoint ? (
              <LeafletMarker position={userPoint} icon={userIcon}>
