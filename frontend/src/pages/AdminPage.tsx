...
type AdminTabKey = 'frontdesk' | 'inventory' | 'housekeeping' | 'maintenance' | 'invoice' | 'rates' | 'promotions' | 'channels' | 'payments' | 'accounts' | 'timeline' | 'reports';
...
export default function AdminPage() {
  ...
  const [accountSnapshot, setAccountSnapshot] = useState<AccountSnapshot>({ system_users: [], guest_accounts: [] });
  const [loadingHotels, setLoadingHotels] = useState(true);
  const [activeTab, setActiveTab] = useState<AdminTabKey>(() => (isAdminUser ? 'frontdesk' : 'rates'));

  const [reportSummary, setReportSummary] = useState<any>(null);
  const [revenueData, setRevenueData] = useState<any[]>([]);
  const [brandRevenueData, setBrandRevenueData] = useState<any[]>([]);
