...
type Hotel = {
  ...
  min_nightly_rate?: number | string | null;
...
type HotelWithCoordinates = HotelWithAvailability & {
  latitude: number | string;
  longitude: number | string;
};

type HotelWithAvailability = Hotel & {
...
function normalizeQueryState(query: Partial<QueryState>): QueryState {
...
function readQueryState(searchParams: URLSearchParams) {
...
function buildQueryParams(query: QueryState) {
...
function hasHotelCoordinates(hotel: Hotel) {
  const latitude = Number(hotel.latitude);
  const longitude = Number(hotel.longitude);
  return Number.isFinite(latitude) && Number.isFinite(longitude);
}

function withCoordinates(hotel: HotelWithAvailability): HotelWithCoordinates {
  return {
    ...hotel,
...
function buildGoogleDirectionsUrl(originLat: number | undefined, originLng: number | undefined, destinationLat: number, destinationLng: number) {
...
function buildSearchHaystack(hotel: Hotel) {
...
function hotelMatchesDestination(hotel: Hotel, destination: string) {
...
function HotelCard({ hotel, checkin, checkout, guests, onPreviewDirections, directionsState, directionsLoading }: {
  hotel: HotelWithAvailability;
  checkin: string;
  checkout: string;
  guests: number;
  onPreviewDirections: (hotel: HotelWithAvailability) => void;
  directionsState?: DirectionState;
      ...
      <div className="search-hotel-body">
        <div className="search-hotel-top">
          <div>
            ...
            <StarRating stars={Number(hotel.star_rating || 0)} />
          </div>
...
export default function SearchPage() {
  ...
  const [availabilityByHotel, setAvailabilityByHotel] = useState<Record<number, AvailabilityEntry>>({});
  const [availabilityLoading, setAvailabilityLoading] = useState(false);
  const [filters, setFilters] = useState<Filters>({ minPrice: '', maxPrice: '', stars: '', brand: '' });
  const [destinationOptions, setDestinationOptions] = useState<string[]>([]);
  const [directionsLoadingHotelId, setDirectionsLoadingHotelId] = useState<number | null>(null);
  const [directionsStateByHotel, setDirectionsStateByHotel] = useState<Record<number, DirectionState>>({});
  const [previewHotel, setPreviewHotel] = useState<HotelWithAvailability | null>(null);
    ...
    apiRequest('/locations')
      .then((res) => {
        ...
        const options = (res.data || [])
          .filter((location: { location_type?: string }) => ['CITY', 'DISTRICT'].includes(location.location_type || ''))
          .map((location: { location_name?: string }) => location.location_name)
          .filter((name: string | undefined): name is string => Boolean(name));
        setDestinationOptions([...new Set<string>(options)].sort((a, b) => a.localeCompare(b)));
      })
      .catch(() => {
  ...
  function updateSearch(updater: Partial<QueryState> | ((prev: QueryState) => QueryState)) {
    setSearch((prev) => {
  ...
  function applyQuery(nextQuery: QueryState, replace = true) {
  ...
  async function handlePreviewDirections(hotel: HotelWithAvailability, forceRefresh = false) {
          ...
          <div className="guests-stepper search-guests-stepper">
            ...
            <input
              ...
              onChange={(event) => {
                ...
                updateSearch((prev) => ({
                  ...prev,
                  guests: Number.isNaN(value) ? 1 : Math.min(10, Math.max(1, value)),
                }));
              }}
            ...
            <>
              {previewHotel ? (
                <SearchDirectionsPreview
                  hotel={withCoordinates(previewHotel)}
