...
//  Category icon map 
type AccountTab = 'Overview' | 'Bookings & Reviews' | 'In-house Services' | 'Loyalty' | 'Profile';

type GuestService = any;
type GuestReservation = any;
type GuestOrder = any;
type GuestProfile = any;
type LoyaltyReward = any;
type LoyaltyRedemption = any;
type BookingReservation = any;
type Review = any;
type Stay = any;

type IssueCategoryKey = 'PLUMBING' | 'ELECTRICAL' | 'HVAC' | 'APPLIANCE' | 'FURNITURE' | 'CLEANING' | 'OTHER';

type IssueForm = {
  issue_category: IssueCategoryKey;
...
const ISSUE_CATEGORIES: { key: IssueCategoryKey; label: string; icon: string; example: string }[] = [
...
const EMPTY_ISSUE: IssueForm = { issue_category: 'PLUMBING', issue_description: '' };
...
function normalizeIssueTicket(ticket: any): NormalizedIssue {
...
function formatMoney(value: number | string | null | undefined, currency = 'VND') {
...
function formatDateOnly(val?: string) {
  if (!val) return '';
...
function GuestServices({ guestId }: { guestId?: number | string }) {
  const { setFlash } = useFlash();

  const [activeReservation, setActiveReservation] = useState<GuestReservation | null>(null);
  ...
  const [reportedIssues, setReportedIssues] = useState<NormalizedIssue[]>([]);
    ...
    async function load() {
      ...
      try {
        ...
        const svcData = svcPayload.data || [];
          setServices(svcData as GuestService[]);
  ...
  async function handleReportIssue(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
  ...
  const grouped = services.reduce<Record<string, GuestService[]>>((acc, svc) => {
      ...
      {selectedService && (
        <form className="guest-svc-order-form" onSubmit={handleOrder}>
          ...
          <div className="guest-svc-order-footer">
            <span>Estimated total: <strong>{formatMoney(Number(selectedService.base_price || 0) * Number(quantity || 0), selectedService.currency_code || hotelCurrency)}</strong></span>
...
function LoyaltyRewardsPanel({ guestId, onProfileRefresh }: { guestId?: number | string; onProfileRefresh?: () => void }) {
  const { setFlash } = useFlash();
  const [loading, setLoading] = useState(true);
...
function BookingReviewsPanel({ guestId }: { guestId?: number | string }) {
  const { setFlash } = useFlash();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const [reservations, setReservations] = useState<BookingReservation[]>([]);
  const [reviews, setReviews] = useState<Review[]>([]);
  const [openReservationId, setOpenReservationId] = useState<number | string | null>(null);
  ...
  const reviewsByReservation = reviews.reduce<Record<string, Review>>((acc, review) => {
    acc[review.reservation_id] = review;
    return acc;
  }, {});

  function openReviewForm(reservation: BookingReservation) {
  ...
  async function submitReview(event: FormEvent<HTMLFormElement>, reservation: BookingReservation) {
                ...
                <div className="acct-review-form-grid">
                  <label>
                    ...
                    <select
                      ...
                      onChange={(event) => setDraft((current) => ({ ...current, rating_score: Number(event.target.value) }))}
                    >
                      {[5, 4, 3, 2, 1].map((score) => (
...
export default function AccountPage() {
  ...
  // Profile edit state
  const [profileEdit, setProfileEdit] = useState<ProfileEdit | null>(null);  // null = not editing
  const [profileSaving, setProfileSaving] = useState(false);
  const [profileMsg,    setProfileMsg]    = useState<ProfileMsg | null>(null); // { type:'ok'|'err', text }
  ...
  async function handleProfileSave(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setGuestProfile((prev: GuestProfile | null) => ({ ...prev, ...payload.data }));
      setProfileEdit(null);
