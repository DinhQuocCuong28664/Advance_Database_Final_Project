...
export default function AdminPromotions({ hotels }: { hotels: Hotel[] }) {
  ...
  const [editing,    setEditing]    = useState<number | string | null>(null); // promo being edited inline
  const [editForm,   setEditForm]   = useState<Promo>({});
  const [savingId,   setSavingId]   = useState<number | string | null>(null);
  const [deletingId, setDeletingId] = useState<number | string | null>(null);

  async function loadPromos(hid?: string) {
    setLoading(true);
    try {
  ...
  async function handleSaveEdit(promoId: number | string) {
    setSavingId(promoId);
    try {
  ...
  async function handleDeactivate(p: Promo) {
