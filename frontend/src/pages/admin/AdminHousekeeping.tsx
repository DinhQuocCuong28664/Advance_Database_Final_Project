...
export default function AdminHousekeeping({ hotels }: { hotels: AnyRecord[] }) {
  ...
  const [staff,     setStaff]     = useState<AnyRecord[]>([]);
  ...
  const [assignTarget, setAssignTarget] = useState<AnyRecord | null>(null);
  const [assignStaff,  setAssignStaff]  = useState('');
  const [assignBusy,   setAssignBusy]   = useState(false);

  const [statusBusy, setStatusBusy] = useState<number | string | null>(null); // task id being updated
  ...
  const loadTasks = useCallback(async (hid: number | string, st = filterRef.current.status, pr = filterRef.current.priority) => {
    const h = hid || hotelId;
    if (!h) return;
    setLoading(true);
    try {
    ...
    } catch (e: any) { setFlash({ tone: 'error', text: e.message }); }
    finally     { setLoading(false); }
  }, [hotelId, setFlash]);

  const loadRooms = useCallback(async (hid: number | string) => {
    if (!hid) return;
    try {
  ...
  async function advanceStatus(task: AnyRecord, newStatus: string) {
