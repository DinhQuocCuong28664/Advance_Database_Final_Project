...
type AnyRecord = any;
const fmtDate = (d?: string) => d ? new Date(d).toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'2-digit', hour:'2-digit', minute:'2-digit' }) : 'Never';
...
function QuickActions({ current, onSet, busy, selfLock = false }: { current: string; onSet: (status: string) => void; busy: boolean; selfLock?: boolean }) {
  return (
...
//  System User Row 
function SystemUserRow({ user, hotels = [], onStatusChange, onEdit, currentUserId }: AnyRecord) {
  const [busy, setBusy] = useState(false);
  const { setFlash } = useFlash();
  const isSelf = String(user.user_id) === String(currentUserId);

  async function handleSet(newStatus: string) {
    setBusy(true);
    try {
...
//  Guest Account Row 
function GuestRow({ guest, onStatusChange }: AnyRecord) {
  const [busy, setBusy] = useState(false);
  const { setFlash } = useFlash();
...
//  Main AdminAccounts 
export default function AdminAccounts({ accountSnapshot, setAccountSnapshot, hotels = [] }: AnyRecord) {
  const { setFlash } = useFlash();
  ...
  async function handleSaveStaff(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    setStaffBusy(true);
    try {
      if (editingUserId) {
