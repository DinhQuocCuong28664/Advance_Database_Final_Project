...
type Guest = {
  reservation_guest_id: number | string;
...
type GuestRowProps = {
  guest: Guest;
  onRemove: (guestRowId: number | string) => void;
};

type AdditionalGuestsProps = {
  reservationId: number | string;
};

const AGE_OPTS: AgeCategory[] = ['ADULT', 'CHILD', 'INFANT'];
const AGE_ICONS: Record<AgeCategory, string> = { ADULT: '', CHILD: '', INFANT: '' };

//  Add Guest Form 
function AddGuestForm({ reservationId, onAdded, onCancel }: AddGuestFormProps) {
  ...
  const set = <K extends keyof GuestForm>(k: K, v: GuestForm[K]) => setForm(p => ({ ...p, [k]: v }));
      ...
      <div className="ag-form-row">
        ...
        <div className="ag-form-field">
          <label>Age Category</label>
            <select className="fd-select" value={form.age_category} onChange={e => set('age_category', e.target.value as AgeCategory)}>
...
//  Guest Row 
function GuestRow({ guest, onRemove }: GuestRowProps) {
...
export default function AdditionalGuests({ reservationId }: AdditionalGuestsProps) {
  ...
  function handleAdded(newGuest: Guest) {
    setGuests(g => [...g, newGuest]);
    setShowForm(false);
  }

  function handleRemoved(guestRowId: number | string) {
