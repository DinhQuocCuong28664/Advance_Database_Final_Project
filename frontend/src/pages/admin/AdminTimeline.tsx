...
function SummaryPills({ summary }: { summary: Record<string, number> | null }) {
      ...
      {entries.map(([key, count]) => {
        ...
        return (
          <div key={key} className="tl-stat-pill" style={{ borderColor: `${cfg.color}55`, background: cfg.bg }}>
            ...
            <span className="tl-stat-count" style={{ color: cfg.color }}>{String(count)}</span>
          </div>
        );
      })}
    </div>
...
export default function AdminTimeline({ hotels = [] }: { hotels?: any[] }) {
  ...
  const groupedAudit = useMemo(() => history.reduce<Record<string, any[]>>((acc, row) => {
