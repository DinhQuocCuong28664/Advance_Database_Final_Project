...
function formatCurrency(value: number | string | null | undefined, currency = 'VND') {
...
export default function AdminInventory({ hotels, loadingHotels }: { hotels: AnyRecord[]; loadingHotels?: boolean }) {
  ...
  const [inventoryRooms, setInventoryRooms] = useState<AnyRecord[]>([]);
  ...
  const summary = useMemo(() => {
    ...
    const byStatus = recs.reduce<Record<string, number>>((a, r: AnyRecord) => {
  ...
  async function handleLoad(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
  ...
  async function handleRecordSave(record: AnyRecord) {
