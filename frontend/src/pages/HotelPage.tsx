...
type Room = {
  ...
  floor_number?: number;
  view_type?: string;
  min_nightly_rate?: number | string;
  nightly_rate?: number | string;
  currency_code?: string;
  availability_status?: string;
  sql_features?: RoomFeature[];
};
...
function maskGuestName(value?: string) {
...
// ── Room image map by category (Unsplash, no emoji) ──────────────────────────
...
function RoomCard({ room, checkin, checkout, onSelect }: { room: Room; checkin: string; checkout: string; onSelect: (room: Room) => void }) {
  ...
  // Carousel
  const roomImgs = ROOM_IMAGES[String(room.category || '').toUpperCase()] || ROOM_IMAGES.DEFAULT;
  const carouselRef = useRef<HTMLDivElement | null>(null);
  useEffect(() => {
    const el = carouselRef.current;
    if (!el || roomImgs.length <= 1) return;
  ...
  return (
    ...
    >
      ...
      <div className="room-card-images" ref={carouselRef}>
        {roomImgs.map((src, idx) => (
          <img
            key={idx}
            src={src}
            alt={`${room.room_type_name || room.room_type || 'Room'} - photo ${idx + 1}`}
            className="room-card-image"
            loading={idx === 0 ? 'eager' : 'lazy'}
          />
        ))}
        {roomImgs.length > 1 && (
          <div className="room-card-images-hint">Swipe for more photos</div>
        )}
      </div>
        ...
        <div>
          ...
          {room.sql_features?.length > 0 && (
            <div className="room-feature-pills">
              {room.sql_features.slice(0, 6).map((feature, index) => (
                <span
...
export default function HotelPage() {
  ...
  const [reviewSummary, setReviewSummary] = useState<ReviewSummary>({ review_count: 0, average_rating: null });
