...
BEGIN
    SET NOCOUNT ON;
    -- XACT_ABORT is intentionally OFF to prevent auto-rollback of the entire Node.js transaction
    SET XACT_ABORT OFF;
    ...
    -- Determine if we are the transaction owner or nested
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            INSERT INTO InventoryLockLog
                (reservation_code_attempt, room_id, stay_date, lock_acquired_at, lock_released_at, lock_status, session_id, transaction_id, note)
            VALUES
        ...
        -- ============================================================
        -- STEP 2: Check availability (Double-Booking caught here)
        -- ============================================================
        IF @current_status <> 'OPEN'
        BEGIN
            SET @result_status  = 2;
            SET @result_message = N'REJECTED: Room not available. Current status: ' + @current_status;
        ...
        INSERT INTO InventoryLockLog
            (reservation_code_attempt, room_id, stay_date, lock_acquired_at, lock_released_at, lock_status, session_id, transaction_id, note)
        VALUES
            (@reservation_code, @room_id, @stay_date, @lock_acquired_at, GETDATE(), 'SUCCESS', @session_id, @transaction_id, N'Room reserved successfully');
