...
| F06.3 | Cập nhật trạng thái | OPEN → IN_PROGRESS → COMPLETED → VERIFIED |
| F06.4 | Đồng bộ trạng thái phòng | Khi VERIFIED thì Room.housekeeping_status = CLEAN |
...
Request
  ...
  │
  └── requireSystemRole(['ADMIN', 'CASHIER'])  ← Kiểm tra role
```
...
[Staff] Nhận task, cập nhật trạng thái
    ...
    ├── PUT /housekeeping/:id/status → COMPLETED
    └── PUT /housekeeping/:id/status → VERIFIED
              │
              └── Auto: Room.housekeeping_status = 'CLEAN'
...
| Reminder check-in | 24h trước ngày nhận phòng |
