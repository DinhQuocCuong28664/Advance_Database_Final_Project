#!/usr/bin/env python3
"""
LuxeReserve - Advanced Database Technical Report Generator (DOCX)
Generates a formal academic technical report document.
"""
...
def create_technical_report():
    ...
    toc = [
        ...
        (2, '2.4', 'Polyglot Persistence Strategy'),
        (1, '3.', 'System Architecture'),
        (2, '3.1', 'Three-Tier Architecture'),
        (2, '3.2', 'Hybrid Data Layer Design'),
        (2, '3.3', 'API Design and Routing'),
        (2, '3.4', 'Authentication and Authorization'),
        (1, '4.', 'Database Design'),
        (2, '4.1', 'Entity-Relationship Model'),
        (2, '4.2', 'SQL Server Schema (30 Tables)'),
        (2, '4.3', 'MongoDB Collections (3 Collections)'),
        (2, '4.4', 'Domain Decomposition'),
        (1, '5.', 'Advanced Database Techniques'),
        (2, '5.1', 'Recursive CTE for Location Hierarchy'),
        (2, '5.2', 'Pessimistic Locking for Concurrency Control'),
        (2, '5.3', 'Optimistic Locking for Inventory Updates'),
        (2, '5.4', 'Window Functions for Revenue Analytics'),
        (2, '5.5', 'Triggers for Audit and Data Integrity'),
        (2, '5.6', 'Stored Procedures for Business Logic'),
        (2, '5.7', 'Views for Financial Reporting'),
        (2, '5.8', 'Computed Columns for Derived Data'),
        (1, '6.', 'System Features'),
        (2, '6.1', 'Reservation Management'),
        (2, '6.2', 'Payment Processing'),
        (2, '6.3', 'Front Desk Operations'),
        (2, '6.4', 'Housekeeping and Maintenance'),
        (2, '6.5', 'Guest Services and Loyalty'),
        (2, '6.6', 'Admin Dashboard and Reporting'),
        (1, '7.', 'Source Code Architecture'),
        (2, '7.1', 'Backend Structure (Node.js/Express)'),
        (2, '7.2', 'Frontend Structure (React/Vite)'),
        (2, '7.3', 'Database Migration Scripts'),
        (2, '7.4', 'Testing Framework'),
        (1, '8.', 'Conclusion'),
        (2, '8.1', 'Achievements'),
        (2, '8.2', 'Challenges and Lessons Learned'),
        (2, '8.3', 'Future Enhancements'),
    ]
    ...
    doc.add_paragraph(
        ...
        'business intelligence reporting. The vw_RevenueByHotel view demonstrates multiple window '
        'function techniques: SUM with OVER and PARTITION BY to calculate monthly revenue per hotel, '
        'SUM with OVER and ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW to compute cumulative '
        'running totals, DENSE_RANK with PARTITION BY to rank hotels within their brand by monthly '
        'revenue, and ratio calculations using SUM OVER with PARTITION BY to determine each hotel\'s '
        'revenue share percentage within its brand. The vw_BookingChannelStats view uses window '
        'functions to calculate each booking channel\'s share of total reservations per hotel. '
        'The admin revenue report endpoint (GET /api/v1/admin/reports/revenue) uses DENSE_RANK to '
        'identify top-performing room types and SUM OVER with PARTITION BY to compute revenue '
        'contribution percentages. These window function-based queries replace what would otherwise '
        'require multiple round-trips or complex application-level aggregation, demonstrating the '
        'power of set-based analytical processing in SQL Server.'
    )
    
    doc.add_heading('5.5 Triggers for Audit and Data Integrity', level=2)
    doc.add_paragraph(
        'LuxeReserve implements three audit triggers that automatically log data changes to the AuditLog '
        'table. The trg_Payment_AuditLog trigger fires on INSERT and UPDATE operations on the Payment '
        'table, logging payment creation events and status changes with full before-and-after JSON snapshots. '
        'The trg_Guest_AuditLog trigger fires on UPDATE operations on the Guest table, capturing profile '
        'changes including email, name, phone number, VIP flag, and identity document modifications. '
        'The trg_GuestAuth_AuditLog trigger fires on UPDATE operations on the GuestAuth table, detecting '
        'password changes, email verification events, and account status transitions. Additionally, the '
        'trg_RoomRate_PriceIntegrityGuard trigger on the RoomRate table automatically inserts a warning '
        'record into RateChangeLog whenever a rate change exceeds 50 percent, with severity level set to '
        'CRITICAL. All triggers are implemented with TRY-CATCH error handling and explicit transaction '
        'management to ensure they do not compromise the integrity of the parent transaction.'
    )
    
    doc.add_heading('5.6 Stored Procedures for Business Logic', level=2)
    doc.add_paragraph(
        'LuxeReserve implements several stored procedures that encapsulate complex business logic '
        'within the database layer. The sp_ReserveRoom procedure is the most critical, implementing '
        'pessimistic locking with UPDLOCK and HOLDLOCK hints to prevent double-booking. It accepts '
        'room_id, stay_date, reservation_code, and session_id as parameters, acquires an exclusive '
        'lock on the RoomAvailability row, checks the current status, updates it to BOOKED if '
        'available, logs the operation in InventoryLockLog, and returns a result message. The '
        'sp_TransferRoom procedure extends this concept for room transfers, atomically releasing '
        'the old room and locking the new room across all nights of the stay. The sp_CreateReservation '
        'procedure orchestrates the entire booking creation process, including guest validation, '
        'room availability checking, rate calculation, reservation header creation, room line item '
        'insertion, and initial status history logging. All procedures use SET XACT_ABORT ON for '
        'automatic rollback on critical errors and include comprehensive error handling with '
        'TRY-CATCH blocks and explicit transaction management.'
    )
    
    doc.add_heading('5.7 Views for Financial Reporting', level=2)
    doc.add_paragraph(
        'The vw_ReservationTotal view serves as the single source of truth for all financial '
        'calculations in the LuxeReserve system. It replaces the previously stored financial fields '
        'in the Reservation table (FIX-3) with computed aggregations from three related tables. '
        'The view joins the Reservation table with aggregated subqueries from ReservationRoom '
        '(room revenue including subtotal, tax, discount, and final amounts), ReservationService '
        '(service revenue including subtotal, discount, and final amounts), and Payment (total '
        'captured payments). The grand total is calculated as the sum of room_final and service_final, '
        'while the balance due is computed as grand_total minus total_paid. This view is used by '
        'the invoice generation system, the admin revenue dashboard, and the reservation detail '
        'endpoint. Additional views include vw_LocationTree for hierarchical location queries, '
        'vw_RevenueByHotel for monthly revenue analytics with window functions, and '
        'vw_BookingChannelStats for channel performance analysis.'
    )
    
    doc.add_heading('5.8 Computed Columns for Derived Data', level=2)
    doc.add_paragraph(
        'The Guest table implements a computed persisted column for full_name that concatenates '
        'first_name, middle_name, and last_name using the CONCAT function with COALESCE to handle '
        'null middle names gracefully. The PERSISTED keyword ensures that the computed value is '
        'physically stored on disk rather than recalculated on every query, which allows the '
        'column to be indexed for efficient search operations. This design decision (documented '
        'as FIX-4 in the project specification) eliminates the need for application-level name '
        'concatenation logic and ensures consistent name formatting across all system interfaces. '
        'The computed column approach was chosen over a stored column to eliminate data redundancy '
        'and the risk of inconsistency between the component name fields and the full name field.'
    )
    ...
    doc.add_heading('6.1 Reservation Management', level=2)
    doc.add_paragraph(
        'The reservation management module is the core of the LuxeReserve system, providing a '
        'complete booking lifecycle from initial creation through check-out. Guests can search for '
        'available rooms by hotel, date range, and occupancy requirements, with real-time availability '
        'displayed through the room search endpoint. The booking creation process involves multiple '
        'database operations wrapped in a single ACID transaction: guest validation, room availability '
        'checking with pessimistic locking, rate calculation from the RoomRate table, reservation '
        'header creation with a unique confirmation code, room line item insertion with rate snapshots, '
        'and initial status history logging. Once created, reservations progress through a defined '
        'status workflow: PENDING (awaiting deposit), CONFIRMED (deposit received), CHECKED_IN '
        '(guest arrived), CHECKED_OUT (guest departed), CANCELLED (by guest or hotel), and NO_SHOW. '
        'Each status transition is logged in the ReservationStatusHistory table, providing a complete '
        'audit trail. The system supports room transfers with atomic lock release and acquisition, '
        'and guest-initiated cancellations with automatic deposit forfeiture.'
    )
    
    doc.add_heading('6.2 Payment Processing', level=2)
    doc.add_paragraph(
        'The payment processing module handles all financial transactions within the LuxeReserve '
        'system. Payments are classified into five types: DEPOSIT (initial booking deposit), '
        'PREPAYMENT (partial payment before arrival), FULL_PAYMENT (settlement of remaining balance), '
        'REFUND (money returned to guest), and INCIDENTAL_HOLD (hold for incidental charges). Each '
        'payment undergoes validation to ensure it does not exceed the allowed amounts: deposits '
        'are capped at the reservation\'s deposit_amount, prepayments and full payments are validated '
        'against the remaining balance calculated from vw_ReservationTotal, and refunds are only '
        'allowed for hotel-initiated cancellations. The system integrates with VNPay for Vietnamese '
        'payment gateway processing, supporting both payment initiation and return URL handling. '
        'Payment statuses include PENDING, CAPTURED, FAILED, REFUNDED, and VOIDED, with each '
        'transition logged through the trg_Payment_AuditLog trigger.'
    )
    
    doc.add_heading('6.3 Front Desk Operations', level=2)
    doc.add_paragraph(
        'The front desk module provides cashier and front desk staff with tools for managing daily '
        'hotel operations. The check-in process updates the reservation status to CHECKED_IN, '
        'creates StayRecord entries for each room with actual check-in timestamps, updates room '
        'occupancy status to OCCUPIED, and logs the status change in ReservationStatusHistory. '
        'The check-out process is more complex, updating the reservation to CHECKED_OUT, recording '
        'actual check-out times in StayRecord, updating room status to VACANT_DIRTY, automatically '
        'creating HousekeepingTask entries for room cleaning, and logging all changes. The cashier '
        'dashboard provides a real-time view of current reservations, pending check-ins and check-outs, '
        'and outstanding balances. The system also supports room transfers, guest lookup by name or '
        'reservation code, and incidental charge posting.'
    )
    
    doc.add_heading('6.4 Housekeeping and Maintenance', level=2)
    doc.add_paragraph(
        'The housekeeping module manages room cleaning tasks with priority levels (LOW, MEDIUM, HIGH, '
        'URGENT) and staff assignment. Tasks are automatically created upon check-out and can be '
        'manually created for special cleaning requests. The task status workflow includes OPEN, '
        'ASSIGNED, IN_PROGRESS, COMPLETED, and INSPECTED, with each transition updating the '
        'associated Room\'s housekeeping_status field. The maintenance module tracks repair tickets '
        'with severity levels (LOW, MEDIUM, HIGH, CRITICAL) and resolution tracking. When a '
        'maintenance ticket is created with HIGH or CRITICAL severity, the system automatically '
        'updates the Room\'s maintenance_status to UNDER_REPAIR. Tickets are only considered resolved '
        'when all open tickets for a room are closed, at which point the room status is reset to '
        'NORMAL. Both modules provide filtering by hotel, status, and assigned staff member.'
    )
    
    doc.add_heading('6.5 Guest Services and Loyalty', level=2)
    doc.add_paragraph(
        'The guest services module allows guests to order ancillary services during their stay, '
        'including spa treatments, in-room dining, airport transfers, butler service, and other '
        'luxury amenities from the ServiceCatalog. Service orders are linked to reservations and '
        'can be paid independently through the incidental payment system. The loyalty program '
        'module manages guest loyalty accounts with tier levels (SILVER, GOLD, PLATINUM, BLACK) '
        'and points accumulation. The LoyaltyRedemption table tracks points redemption, linking '
        'guests, promotions, and reservations. Guests can redeem points for promotional vouchers '
        'with configurable point costs and validity periods. The system also supports guest reviews '
        'and ratings through the HotelReview table, with moderation workflow for quality control.'
    )
    
    doc.add_heading('6.6 Admin Dashboard and Reporting', level=2)
    doc.add_paragraph(
        'The admin dashboard provides comprehensive business intelligence and system management '
        'capabilities. Revenue analytics reports use window functions to calculate monthly revenue '
        'per hotel, cumulative running totals, and revenue share percentages across brands and '
        'chains. The Price Guard system monitors rate changes and alerts administrators when '
        'changes exceed 50 percent through the RateChangeLog table. Inventory management allows '
        'administrators to update room availability with optimistic locking for concurrency control. '
        'The invoice generation system creates financial invoices from the vw_ReservationTotal view, '
        'ensuring that all invoices are based on the same financial calculation logic. The admin '
        'interface also provides user management for staff accounts, role assignment, and system '
    ...
    # ============================================================
    # CHAPTER 7: SOURCE CODE ARCHITECTURE
    # ============================================================
    doc.add_heading('7. Source Code Architecture', level=1)
    
    doc.add_heading('7.1 Backend Structure (Node.js/Express)', level=2)
    doc.add_paragraph(
        'The backend source code is organized into a modular structure under the src/ directory. '
        'The main entry point is src/app.js, which initializes the Express application, configures '
        'middleware (CORS, JSON parsing, JWT authentication), and mounts the 14 route groups. '
        'The src/config/ directory contains database connection configurations for both SQL Server '
        '(using mssql with msnodesqlv8 driver) and MongoDB Atlas (using the official MongoDB driver). '
        'The src/middleware/ directory implements the authentication middleware stack including '
        'attachAuthContext, requireAuth, and requireSystemUser functions. The src/routes/ directory '
        'contains 14 route modules, each implementing CRUD operations and business logic for its '
        'domain. The src/services/ directory contains shared service modules including mail.js '
        '(email notifications via Nodemailer) and vnpay.js (VNPay payment gateway integration). '
        'The database/ directory contains all SQL migration scripts (numbered 01 through 24) and '
        'MongoDB seed scripts, providing a complete database setup pipeline.'
    )
    
    doc.add_heading('7.2 Frontend Structure (React/Vite)', level=2)
    doc.add_paragraph(
        'The frontend source code is organized under the frontend/src/ directory. The main entry '
        'point is main.jsx, which renders the App component wrapped in AuthProvider and FlashProvider '
        'contexts. The App.jsx file defines the routing structure using React Router v6, mapping '
        'URL paths to page components. The pages/ directory contains 12 page components: '
        'DashboardPage (home), SearchPage (hotel search), HotelPage (hotel details), BookingPage '
        '(booking flow), ReservationPage (reservation lookup), LoginPage, RegisterPage, '
        'ForgotPasswordPage, ResetPasswordPage, AccountPage (guest profile), AdminPage (admin '
        'dashboard), CashierPage (front desk operations), and VnpayReturnPage (payment callback). '
        'The components/ directory contains reusable UI components including layout components '
        '(header, footer, navigation) and map components. The context/ directory contains '
        'AuthContext.jsx for authentication state management and FlashContext.jsx for global '
        'notification toasts. The lib/ directory contains the API client module (api.js) that '
        'handles HTTP requests with automatic JWT token attachment. The hooks/ directory contains '
        'custom React hooks including useApi for API call management. The utils/ directory contains '
        'utility modules for data formatting and hotel image management.'
    )
    
    doc.add_heading('7.3 Database Migration Scripts', level=2)
    doc.add_paragraph(
        'The database migration scripts are organized as numbered SQL files in the database/sql/ '
        'directory, designed to be executed in sequence. Script 01 creates the database, 02 creates '
        'all 30 tables with constraints and indexes, 03 creates views (vw_ReservationTotal, '
        'vw_LocationTree, vw_RevenueByHotel, vw_BookingChannelStats), 04 creates triggers '
        '(trg_Payment_AuditLog, trg_Guest_AuditLog, trg_GuestAuth_AuditLog, '
        'trg_RoomRate_PriceIntegrityGuard), 05 creates stored procedures (sp_ReserveRoom, '
        'sp_TransferRoom, sp_CreateReservation), and 06 seeds demo data. Subsequent scripts add '
        'advanced features: 20 adds loyalty rewards, 21 adds manager roles, 22 adds hotel reviews, '
        '23 adds advanced stored procedures, and 24 adds audit triggers. The MongoDB seed scripts '
        'in database/mongodb/ populate the Hotel_Catalog, room_type_catalog, and amenity_master '
        'collections with rich content for the demo hotels. The database/reset_and_setup.js script '
        'provides a complete database reset and setup workflow.'
    )
    
    doc.add_heading('7.4 Testing Framework', level=2)
    doc.add_paragraph(
        'The testing framework is built using Playwright for end-to-end testing and custom Node.js '
        'scripts for API-level testing. The tests/ directory contains test scripts for concurrent '
        'booking scenarios (test_concurrent_booking.js) and reservation payment flow validation '
        '(test_reservation_payment_flow.js). The tests/api/ directory contains API-level test '
        'suites. The Playwright configuration (playwright.config.js) defines browser launch options '
        'and test environment settings. The test-results/ directory stores test execution reports '
        'including the Playwright HTML report. The check_remaining.py script provides automated '
        'validation of remaining project requirements, and the remaining_audit.txt file tracks '
        'audit findings. The testing strategy covers unit testing of database procedures, '
        'integration testing of API endpoints, and end-to-end testing of critical user workflows '
        'including the complete booking and payment flow.'
    )
    ...
    doc.add_paragraph(
        'The LuxeReserve project successfully demonstrates the practical application of advanced '
        'database techniques in a real-world hotel reservation management system. The project '
        'achieved all of its primary objectives: implementing ACID-compliant transactions with '
        'pessimistic locking to prevent double-booking, designing a polyglot persistence architecture '
        'combining SQL Server and MongoDB, creating a comprehensive set of 82 RESTful API endpoints, '
        'building an intuitive React-based user interface for three user groups, and implementing '
        'automated audit logging through database triggers. The system\'s 30 SQL Server tables and '
        '3 MongoDB collections are organized into 12 logical domains, demonstrating sophisticated '
        'database design principles including normalization, denormalization for performance, '
        'computed columns, CHECK constraints, and strategic indexing. The advanced database techniques '
        'implemented include recursive CTEs for hierarchical queries, pessimistic and optimistic '
        'locking for concurrency control, window functions for analytical processing, triggers for '
        'automated auditing, stored procedures for complex business transactions, and views for '
        'financial reporting. The project also includes comprehensive documentation, test scripts, '
        'and a complete database setup pipeline.'
    )
    
    doc.add_heading('8.2 Challenges and Lessons Learned', level=2)
    doc.add_paragraph(
        'Several significant challenges were encountered during the development of LuxeReserve. '
        'The most technically demanding challenge was implementing pessimistic locking correctly '
        'to prevent double-booking while avoiding deadlocks in high-concurrency scenarios. This '
        'required careful study of SQL Server locking mechanisms, including the distinction between '
        'UPDLOCK and HOLDLOCK hints and their interaction with transaction isolation levels. '
        'Another challenge was designing the hybrid data layer to efficiently merge data from SQL '
        'Server and MongoDB without introducing excessive latency or complexity. The solution '
        'involved implementing parallel queries with in-memory Map-based merging at the API layer. '
        'A third challenge was ensuring data consistency across the two database systems, which '
        'was addressed by clearly defining SQL Server as the system of record for all transactional '
        'data and MongoDB as a read-optimized content cache. The project also highlighted the '
        'importance of comprehensive error handling in database triggers and stored procedures, '
        'as unhandled errors in these objects can compromise the integrity of parent transactions.'
    )
    
    doc.add_heading('8.3 Future Enhancements', level=2)
    doc.add_paragraph(
        'Several enhancements are planned for future versions of LuxeReserve. Integration with '
        'third-party Online Travel Agencies (OTAs) such as Booking.com and Agoda would expand '
        'the system\'s reach and allow centralized inventory management across multiple distribution '
        'channels. A native mobile application for both guests and staff would improve accessibility '
        'and enable push notifications for booking confirmations and check-in reminders. Artificial '
        'intelligence-based revenue management could optimize room pricing dynamically based on '
        'demand forecasts, competitor pricing, and historical booking patterns. Multi-property '
        'consolidated accounting would provide enterprise-level financial reporting across all '
        'hotels in a chain. Enhanced data analytics with real-time dashboards and predictive '
        'modeling would provide deeper business intelligence insights. Finally, integration with '
        'property management systems (PMS) and point-of-sale (POS) systems would create a fully '
        'integrated hospitality technology ecosystem.'
    )
    
    # ============================================================
    # SAVE
    # ============================================================
    output_path = 'LuxeReserve_Advanced_Database_Technical_Report.docx'
    doc.save(output_path)
    print('[OK] Technical report saved to: %s' % output_path)
    return output_path
